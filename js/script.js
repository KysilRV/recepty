...
window.addEventListener('DOMContentLoaded', () => {
    ...
    const listIngred = [
        ...
        {
            name: 'Капуста',
            ingred: ['вода: 1000г', 'уксус: 150г', 'сахарa: 150г', 'масло(растительное): 150г', 'соль: 2ч'],
            value: 3000,
            keyItem: 'Капуста',
            descr: 'честнок, морковка'
        }
    ];
    ...
    if (window.location.href.split('/')[window.location.href.split('/').length - 1] === '' ||
        ...
        const listOfVal = [];
            ...
            listOfVal.forEach(item => {
                ...
                tmp.push((n * +localStorage.getItem('inputValue')).toFixed(2));
            });
        ...
        inputOfVal.addEventListener('input', e => {
            ...
            listOfVal.forEach(item => {
                ...
                tmp.push((n * +target.value).toFixed(2));
            });
        ...
        inputFirst.addEventListener('input', e => {
            ...
            if (listOfAddedIngred.children && firstVal.value) {
